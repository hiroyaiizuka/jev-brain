 ...
 *
 * - 除いた結果が空になるときは入力をそのまま返す。空の配列を渡すと Excalidraw が合わせる先を失う。
 *   3D にノードが 1 つも無い画面は起きない（中心ノートは必ず描く）が、床だけを画面いっぱいに広げるより
 *   これまでの挙動に倒すほうが安全。
 * - `sceneryIds` が空（2D）なら入力をそのまま返す。呼び出し側（`Scene.zoomToFitNodes`）は 2D をここへ渡さず
 *   上流の対象をそのまま使うが、id の集合だけを見て答えが決まる関数にしておく。
 */
