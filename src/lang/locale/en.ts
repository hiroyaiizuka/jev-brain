...
export default {
  ...
  JEV_RELATIONS_HEADING_DESC: "Heading of the section a confirmed field:: [[link]] line is appended to while the setting below writes into a section. The section is added at the end of the note when it is missing. Write the text only: the # markers are added for you, and emptying this restores the default.",
  JEV_WRITE_MODE_NAME: "Where the field is written",
  JEV_WRITE_MODE_DESC: "Inline puts the field on the link you pointed at: field:: [[link]] when the link is alone on its line, (field:: [[link]]) in the middle of a sentence. Another link to the same note elsewhere in the body is left alone. The Relations section instead leaves the body untouched and is easy to read in a diff; a link written as [label](note.md) always goes there, because the field belongs outside a markdown link.",
  JEV_WRITE_MODE_RELATIONS: "Relations section",
  JEV_WRITE_MODE_INLINE: "Inline, on the link (default)",
  ...
  JEV_COMMAND_UNCHANGED: "Nothing was written for {field}:: [[{target}]]: that link already carries a field.",
  JEV_COMMAND_LINK_GONE: "Nothing was written for {field}:: [[{target}]]: the link is no longer where the cursor was. Put the cursor on it again.",
