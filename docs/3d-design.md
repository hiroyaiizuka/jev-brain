...
### 6-8. 帯に残る level 0 だけで列を組み直す（LEV-145）
...
- 軸へ抜けたノードが無い帯（Up／Down を使っていない Vault）も同じ規則で揃える。上流の半端な行は中心に対して非対称なので、穴が開いていなくても床の十字からは同じだけずれる。2D の並びは 3D の分岐の外なので変わらない。
- 2D は変更なし。3D をオフにすると元の座標に戻る。
...
- 床に最低の広がりを課したので、ノードが少なくても床が大きく、`zoomToFit` の倍率が下がる（実測 50% → 42% → 35%）。LEV-144 で `zoomToFit` の対象から床・グリッド・十字・方角（`render3D()` の戻り値）を id で除き、ノードとリンクだけに合わせた（`Projection` とは別の純関数 `src/graph/zoom.ts` の `zoomTargets`。2D は除くものが無いので従来どおり）。床と方角はビューポートからはみ出してよい。**倍率が 2D と揃ったかは未計測**で、実機で確かめる。床が広いこと自体と、左右の釣り合い（LEV-135）が床を広げる側にしか動かないことは変えていない。床の広さを足元の実際の散らばりから決めれば、はみ出す量も一緒に減らせる（LEV-144 のレビューで挙がった）。
- ズームの対象から床を外したので、ノードが少ない画面では倍率が `maxZoom`（既定 1.0）まで上がり、床の縁と N／S／W／E がすべて画面の外に出うる。そうなると「どちらが北か」「どの箱が床の上か」の手がかりが消える。床か方角が少しは見える下限を入れるか、3D だけ `maxZoom` を下げるかは決めていない。まず本人が実機で見て決める。LEV-144 のレビューで挙がった。
