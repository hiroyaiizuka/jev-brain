...
- ~~床に最低の広がりを課したので、ノードが少なくても床が大きく、`zoomToFit` の倍率が下がる~~ 外した（LEV-144）。`zoomToFit` はノードとリンクだけに合わせ、床・グリッド・十字・方角（`render3D()` の戻り値）は id で対象から除く（`Projection` とは別の純関数 `src/graph/zoom.ts` の `zoomTargets`）。床と方角はビューポートからはみ出してよい。2D は除くものが無いので従来どおり。床が広い（実測 50% → 42% → 35%）こと自体と、左右の釣り合い（LEV-135）が床を広げる側にしか動かないことは変えていない。
