...
- 実装（LEV-120）: 床の平面図は `Projection.floorPlan`（2D の地面座標。全ノードの影の地面の位置と中心ノートの足元を囲む最小の長方形＋四方 nodeHeight、グリッドは中心の足元から nodeHeight 間隔で外周の内側だけ、方角は十字の両端の外側 nodeHeight/2）で、Scene が各点を `project(·, floor, params)` で床の高さに置く。浮いたノードの影の位置は 2D の中心そのもの、床のノードの接地影は箱の下端に上端が接する位置（画面座標。箱の背景が半透明なので箱の下に潜らせない）なので `unproject` で地面座標に戻してから床に含める（箱が高いほど床が南へ伸びる。埋め込みの中心が床にいるときは大きく伸びる）。目盛りは `pillarTickLevels`＝床と箱の段の間の各段（箱の段は箱に隠れるので引かない。3 段のままでは Up の親が床 −1 のときの 1 本だけ）。影は幅 nodeHeight × 0.6・高さ × 0.25 の楕円。床・グリッド・十字・柱・影・方角の色はどれもテキスト色（`baseNodeStyle.textColor`）から不透明度だけ落として作る（外周 25%・面 5%・グリッド 15%・十字 40%・影 40%・柱 60%・方角 60%）。
