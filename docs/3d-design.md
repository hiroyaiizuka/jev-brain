...
- 東西は行ごとの中央揃え（帯の中心線 ＝ `LayoutSpecification.origoX`）。満杯の行は `Layout.place()` と同じ位置になり、半端な行は `verticalSpread` と同じ規則で中心を挟んで対称に並ぶ。上流の行ベクトル（`Layout.layout()` の `getRowLayout`）は使わない: 半端な行を列の端へ振り分ける作りで、2 列に 1 つだと中央の null が落ちて西へずれる。
- 南北は読み順（北から南）のまま `rowHeight` 間隔。中心にいちばん近い行は `place()` が置いた位置のままで、丸ごと空いた行のぶんだけ帯が外側で短くなる。中心側の縁が動かないので、続く `bandShift`（§6-6）が測る距離は変わらない。
...
- 軸へ抜けたノードが無い帯（Up／Down を使っていない Vault）は触らない。穴が開いていないなら 2D の格子がそのまま正しい。
