...
- 床の左右は中心ノート（＝ Up／Down の垂直軸）に対して対称にする（LEV-135）。平行四辺形は north のぶん右上に傾くので、東西の範囲をそのまま投影すると左右の端（南西と北東の角）が中心から等距離にならない。`floorPlan` が足りない側へ広げる（狭めない）。足元が東西に対称なら広げる量は `(south − north) × northShearX`（既定値では約 42px 西。床の中心が動く量はその半分）。足元やラベルの長さが東西に偏っている Vault では、その偏りも同じ式で打ち消すので床はさらに広くなる。
...
- 左右の釣り合いは「投影後の南西の角と北東の角を中心から等距離にする」定義にした（LEV-135）。平行四辺形の中心線そのものは傾いているので、代わりに「中心ノートの行（東西軸）で西端と東端を等距離にする」定義も選べる。今の定義では十字の東西の腕と W／E ラベルが軸に対して非対称のまま（既定値で西が約 42px 長い）。本人の目視で決める。
