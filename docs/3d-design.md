...
- 実装（LEV-120）: 床の平面図は `Projection.floorPlan`（2D の地面座標。足元＝各ノードの 2D の中心と中心ノートの足元を囲む最小の長方形＋四方 nodeHeight、グリッドは中心の足元から nodeHeight 間隔で外周の内側だけ、方角は十字の両端の外側 nodeHeight/2）で、Scene が各点を `project(·, floor, params)` で床の高さに置く。目盛りは `pillarTickLevels`＝床と箱の段の間の各段（箱の段は箱に隠れるので引かない。3 段のままでは Up の親が床 −1 のときの 1 本だけ）。影は幅 nodeHeight × 0.6・高さ × 0.25 の楕円。床・グリッド・十字・柱・影・方角の色はどれもテキスト色（`baseNodeStyle.textColor`）から不透明度だけ落として作る（外周 25%・面 5%・グリッド 15%・十字 40%・影 40%・柱 60%・方角 60%）。埋め込みの中心の箱は余白より高いので、床にいるときの接地影が床の外に出る（LEV-123 と同じ原因）。
