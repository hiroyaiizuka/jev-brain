...
y = -(north · northRise) - level · levelHeight  既定 0.30。中心ノートの箱が原点。床は常に中心の段（level 0）の少し下（§6-2）
...
- 実装（LEV-120）: 床の平面は `Scene.floorPlane`＝`project(·, FLOOR_LEVEL, params)` を画面で `drop` px 下げたもの（`drop` = 中心の箱の高さの半分＋影の高さの半分。箱が nodeHeight より高い埋め込みの中心は nodeHeight/2 で頭打ち、LEV-123）。床の平面図は `Projection.floorPlan`（2D の地面座標。全ノードの足元＝2D の中心と中心ノートの足元を囲む最小の長方形＋四方 nodeHeight、グリッドは中心の足元から nodeHeight 間隔で外周の内側だけ、方角は十字の両端の外側 nodeHeight/2）で、Scene が各点を床の平面に置く。影は全ノード足元に（中心の箱の下端に上端が接し、友はその少し上）。目盛りは `pillarTickLevels`＝床と箱の段の間の各段（箱の段は箱に隠れるので引かない。3 段のままでは常に無く、§7 で段が増えたときに効く）で床と同じだけ下げる。影は幅 nodeHeight × 0.6・高さ × 0.25 の楕円。床・グリッド・十字・柱・影・方角の色はどれもテキスト色（`baseNodeStyle.textColor`）から不透明度だけ落として作る（外周 25%・面 5%・グリッド 15%・十字 40%・影 40%・柱 60%・方角 60%）。床は薄い面（5%）なので床の下の子は透けて見える。
...
- ノードの背景色を level 別にする（設定 `levelColors`、既定は模式図の 4 段: L1 淡 → L4 濃）。箱の肩に「L1」のようなラベルを小さく付ける。表示するレベルは `level − 最下段 + 1`（最下段 `floorOf` が L1。床の下に吊る Down の子があれば、その子が L1 で中心が L2）。
