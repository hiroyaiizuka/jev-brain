...
- `Projection` の斜投影（LEV-119。上の `project` の回転・`compressBands`・`groundLevelOf` の記述はこれで置き換え。`compressBands`／`extentOf` は削除、`groundLevelOf` は `floorOf`）: 3d-brief §7 の 8 ノートと `artifacts/3d1-e2e` の実座標で、`DEFAULT_VIEW_3D_SETTINGS`（0.40／0.30／2.2）、if-then／中心／意志力の y が等しいこと、中心が原点に留まること、親は右上・子は左下（影＝`floor` の高さの足元も同じ向き）、隣り合う level の差が `levelHeight`、`depth` が north で level に依らないこと、`floorOf` が最小 level（空と +1 だけなら 0）、`compareDrawOrder` が north 降順・同値は x 昇順で実座標を 親 → 友 → 中心 → 子 に並べること、`boundsOf` を固定する。
