...
- `ToolsPanel` の 3D トグル（LEV-114: `ea.DEVICE?.isDesktop` のときだけ作る `ToggleButton`、`scene.view3D` の切替、`setVal` が false を返して保存しないこと）は Obsidian の DOM（`createDiv`／`setIcon`）と `PageSuggest`／`LinkTagFilter` に依存するので実機のみ（LEV-115: トグルで 3D／2D が切り替わり、再起動後は 2D）。
