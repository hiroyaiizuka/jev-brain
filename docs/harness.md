...
- 初期ズームの対象（LEV-144）: `zoomTargets`（`src/graph/zoom.ts`）が 3D の床・グリッド・十字・方角の id を外してノードとリンクだけを返すこと、2D（床の id が空）と、床の id が画面に無いとき、外すと空になるときは入力をそのまま返すこと、入力を書き換えないことを固定する。`zoomToFit` を呼ぶ `Scene.zoomToFitNodes()` は EA 依存で実機のみ（倍率は CDP の `getAppState().zoom.value` を 2D／3D で比べる）。
