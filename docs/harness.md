...
| E22 | （LEV-185）既定のインライン書き込み: 文中の `[[X]]` にコマンド、行にリンクだけの `[[Y]]` に `]]` のサジェスト、同じ相手が 2 つある本文でキューの確定 | 文中は `(field:: [[X]])`、リンクだけの行は `field:: [[Y]]`、書き換わるのは入口が指した出現だけ。`## Relations` は増えない。`jev-log.json` から取り消せる。未実施 |
