...
| E18 | （JEV-1）設定画面の「Jev」節を開いて API キーを入れ、有効化してタブを閉じ、Obsidian を再読込してから `Fixtures/習慣はトリガー固定で続く` の本文の未型付けリンクにカーソルを置いてコマンド「Jev: カーソルのリンクに型を付ける（第一候補で確定）」 | 設定（LEV-165）: 「Jev」節に注意書きと 10 個の項目が並び、API キーは伏せ字で、有効化して閉じると再読込を促す Notice が 1 回出る。キーを空にして再読込すると Jev の項目が何も登録されない。コマンド（LEV-170）: `## Relations` に `field:: [[X]]` が 1 行入り、本文は変わらない。JevBrain を開くと次の描画で X が型の位置に移る。`jev-log.json` に 1 件。確認 2026-09-22（main 88ba18d、CDP、`artifacts/jev-1-e2e/record.md`）: 1 回目は Jev の実物の応答（`answers`・`type`・`usage.input_tokens`）を `client.ts` が読めず失敗（`e18-raw-response.json`）。PR #32 で実物に合わせて再実行し PASS: `down:: [[読書メモのテンプレート]]`（第一候補 40%、方向の Choice と一致）が `## Relations` に 1 行入り、本文は変わらず、`jev-log.json` に 1 件、再描画で推論の子が Down の定義済みになって南に緑で出た。Notice まで 760 ms、1,709 入力トークン（約 0.011 円）、console.error なし。設定画面の目視（注意書き・伏せ字・再読込の Notice）は本人。手順書は `plan.md` |
| E19 | （JEV-2）JevBrain の view を閉じたままノートに `[[X]]` を書いて `]]` を閉じる | サジェストに候補・確率・方向が出て（確率順で最大 5 件、0% の候補は出ない）、Enter で `## Relations` に追記、Esc で消える。同じリンクで 2 度目は出ない。未実施 |
| E20 | （JEV-3）ツールパネルの Jev ボタンでキューを開き、中心を変える | 中心ノートの未型付けが並び、候補ボタンは確率順で最大 5 件（0% は出ない。自信なしは既定の選択なし）、確定でノードが移り、取り消しで戻る。下部に呼び出し回数と概算費用。未実施 |
| E21 | （JEV-4）`Fixtures/big/大きな脳` を中心に「一括」→ Vault 全体 | 件数と概算費用が出て、実行後に確定 n・確認待ち m と「この一括を取り消す」。取り消しで全行が消える。見直しタブに型付きの提案だけが出る。未実施 |
| E22 | （LEV-185）既定のインライン書き込み: 文中の `[[X]]` にコマンド、行にリンクだけの `[[Y]]`（とリスト項目）に `]]` のサジェスト、同じ相手が 2 つある本文でキューの確定、markdown リンク `[Z](Z.md)` にコマンド | 文中は `(field:: [[X]])`、リンクだけの行は `field:: [[Y]]`（リスト記号は残る）、書き換わるのは入口が指した出現だけで同じ相手の他の出現は変わらない。markdown リンクだけは本文を変えずに `## Relations` に `field:: [[Z.md]]` が 1 行入る。ほかに `## Relations` は増えない。`jev-log.json` から取り消せる。設定の「Where the field is written」の既定が Inline であることも見る。手順書は `artifacts/jev-inline-e2e/plan.md`。確認 2026-09-22（main 3104e2c、CDP、`artifacts/jev-inline-e2e/record.md`）: 文中は `(down:: [[X]])`、リンクだけの行は `down:: [[X]]`、リスト項目は `- down:: [[X]]`、同じ相手が 2 回ある本文はカーソルの 2 つ目だけ。`## Relations` は増えず、`jev-log.json` に 4 件、console.error なし。コマンド経路のみ（サジェスターとキューからは E19／E20 で）。訂正（2026-09-23）: リスト項目の `- down:: [[X]]` は文字列としては正しいが Dataview がページのフィールドにしないので、JevBrain では推論のまま（LEV-193）。文中と行だけの形は定義済みになる |
