...
現在の実装（LEV-185）: 既定の書き込みをインラインにした（`DEFAULT_JEV_SETTINGS.writeMode = "inline"`。設定の選択肢もインラインを先頭にし、説明を入れ替えた。保存済みの `data.json` が `relations` を選んでいればそのまま節に書き続ける＝移行はしない）。`appendRelation` は `at?: { line, ch }`（0 始まり、リンクの `[` の位置）で入口が指した出現だけを書き換え、行頭（字下げとリスト記号 `-`／`*`／`+`／`1.`／`1)` を除く）から行末までがそのリンク 1 つだけなら括弧なしの `field:: [[X]]`、文中なら `(field:: [[X]])` にする。位置を渡さない呼び出し（カーソルを持たない JEV-4 の一括）は今までどおり最初の型の付いていない出現。入口が markdown リンク（`[B](notes/B.md)`）を指したときはインラインにせず `## Relations` 節に `[[…]]` で 1 行足す（設計 §3）。戻り値の `null` は `RelationEdit | { skipped: "already-typed" | "not-found" }` に割り、「既にフィールドが付いている」と「指していたリンクがもう無い」を Notice でも分けた（`JEV_COMMAND_UNCHANGED`／新しい `JEV_COMMAND_LINK_GONE`、キューは `JEV_QUEUE_NO_CHANGE`／`JEV_QUEUE_LINK_GONE`）。位置は 3 入口とも渡す: コマンドは `writtenLinkAt` の `start` とカーソル行、サジェスターは閉じた `]]` の `[[` の位置、キューは `collectUntypedLinks` の `line`／`ch`（`JevQueueCard` に足した）で、キューの書き込みに使うリンクの綴りは本文がその出現を `[[…]]` で書いていればその綴りのまま（`writeTextOf`）。`tests/jev/relations.test.ts`（36 件）が括弧の有無・リスト記号・指した出現・境目・埋め込み・markdown リンクの落とし先・2 つの skip を、`tests/jev/typeLink.test.ts` が 3 つの新しい経路と `link-gone` を、`tests/suggesters/jev-link-suggest.test.ts` が既定のインライン確定・2 つ目の出現・`relations` 指定・リンクが動いた場合を固定する。実機は未実施（E22）。
