...
現在の実装（LEV-170）: コマンド `excalibrain-jev-type-link`（英語 UI で "Jev: type the link at the cursor (confirm the first candidate)"、日本語 UI で「Jev: カーソルのリンクに型を付ける（第一候補で確定）」）を `registerJev()` が登録し、`src/jev/typeLink.ts` の `typeLinkAtCursor` が中核を順に通す（カーソルの `[[X]]` を取り → `collectUntypedLinks` で未型付けか確かめ → `buildState`／`buildQuestions` → `askJev` → `judge` → `confident` なら `appendRelation` と `appendLogEntry`（`source: "command"`）、自信なしは確率を伏せた候補を Notice に出して何も書かない）。書くのは本文のとおりの `[[X]]`（別名と `#見出し` は落とし、`[[…]]` に入れると意味の変わる markdown リンクだけは解決したパス）、フィールドは Jev の答えではなく設定の綴り。埋め込み・型付き・カーソルがリンクの外・除外パスや図面ファイル自身は何もしない。UI は `src/Components/JevTypeLinkCommand.ts` の Notice 1 本だけで、判定中の押し直しは断り、書く直前に `MarkdownView.save()` でエディタのバッファをファイルへ流す（読むのはバッファ、書くのは `vault.process`）。文言は `en`／`ja` の `JEV_COMMAND_*`。JevBrain を一度も開いていない Vault では索引をここで作る。`tests/jev/typeLink.test.ts`（14 件。応答は `tests/fixtures/jev/two-choice-200.json`）が追記・記録・送った state と 2 問・別名・markdown・未解決・フィールドの綴り・自信なし（方向の食い違いとオントロジーに無い語）・応答なし・重複・カーソル外・埋め込み・除外・索引なし・保存の順を、`tests/components/jev-type-link-command.test.ts`（7 件）が Notice の文言を固定する。実機（E18）は本人の API キー待ちで未実施。手順書は `artifacts/jev-1-e2e/plan.md`。
