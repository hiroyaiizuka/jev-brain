...
現在の実装（LEV-187）: `judge` の `ordered` が確率順の上位 5 件（`MAX_CANDIDATES`）までになり、四捨五入で 0% になる候補（`MIN_CANDIDATE_PROBABILITY` ＝ 0.005 未満と、応答が確率を返さなかったフィールド）は出さない。自信なしでも並びも確率も同じで、無くなるのは既定の選択だけ（キューは「Pick a field first」、サジェスターは行に「Jev is not confident」、コマンドは候補を並べて何も書かない）。付け替えの「現在のフィールド」はその 5 件の 1 つとして先頭に入り、候補に無い答えには既定を作らない（`queue-model.ts` の `firstChoice`。別のフィールドを選んだ状態にすると押すだけでJev が答えていない型が入る）。サジェスター・キュー・コマンドの Notice は `ordered` をそのまま出し、自分で並べ替えも絞り込みもしない。`tests/jev/judge.test.ts`（8 件→上位 5 件、0.4% の除外と 0.5% の残留、5 件未満は全部、自信なしも確率順、`maxCandidates`／`minProbability`、しきい値は絞る前の`probabilities` から読む）・`tests/suggesters/jev-link-suggest.test.ts`（自信なしの並びと確率、最大 5 件、行の描画）・`tests/jev/queue-model.test.ts`（既定なし）が固定する。実機は E19／E20 に「候補は最大 5 件で 0% は出ない」を足した（未実施）。
