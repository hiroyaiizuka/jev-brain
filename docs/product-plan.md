...
現在の実装（LEV-168）: `src/jev/collect.ts` の `collectUntypedLinks(app, page, file, content)` と `collectTypedLinks(…)` が `metadataCache.getFileCache(file).links` と `Page.neighbours` からノート本文の `[[X]]` を未型付け（定義済みの関係が無い相手）と型付き（現在のフィールドを `fields` に付ける）に分け、埋め込み（`embeds` にあるので読まない）・URL・自分自身・`excludeFilepaths`・hidden のフィールドで結ばれた相手・同じ相手の 2 件目以降を落とす。返り値は `{ target, displayText, line, ch, context }`（`target` は `Pages` と同じ鍵＝解決したパスか未解決ならリンクの文字列、`context` は引数の本文から取ったその行）で、Dataview の索引は遅いので `Page.getNeighbours()` と同じく自分と隣の `addDVFieldLinksToPage()` を先に呼ぶ（view を開かずに使うサジェスターのため）。`tests/jev/collect.test.ts` が vault／Dataview スタブで受入条件のケース（未型付け 3・型付き 2・埋め込み 1・URL 1）と、別名・見出し付き・未解決・markdown リンク・重複・自己リンク・除外パス・hidden・推論だけの関係・隣の側が定義した関係を固定する。
