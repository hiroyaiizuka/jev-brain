...
現在の実装（LEV-172）: コマンド `excalibrain-jev-suggest-link`（英語 UI で "Jev: type the link at the cursor (pick from the candidates)"、日本語 UI で「Jev: カーソルのリンクに型を付ける（候補から選ぶ）」、既定のホットキーなし。`src/Components/JevSuggestLinkCommand.ts`）が `JevLinkSuggest.openAt` を呼び、カーソル上の本文の `[[X]]`（リンクの中ほどでも可。`]]` の直後・セッション中 1 回・設定 `suggestOnLinkClose` は掛けない）に同じ候補を出す。このノート自身の hidden 以外のフィールドが既にその相手を指していれば（`collectTypedLinks` と同じ判断を、ブレインを開いていなくても動くよう Dataview で 1 件だけ見る）、そのフィールドを `judge` の `currentField` で先頭に置き（行に「current」）、確定は `replaceRelation`（今のフィールドを選べば何も書かない。本文に `current:: [[X]]` の行が無ければ書かずに Notice）、記録は `source: "suggest"`。相手のノートだけが型を付けている・hidden で結ばれている・埋め込み・リンクの外・除外ノートでは開かず、理由を Notice（`JEV_SUGGEST_*`）に出す。カーソルがリンクを離れるとポップアップは閉じる。第一候補で確定するコマンド（LEV-170）は残した。`tests/suggesters/jev-link-suggest.test.ts`（型付き／未型付けで `ordered` の先頭が変わる、付け替えの書き込みと記録、同じフィールドを選んだとき、行が無いとき、設定オフでも開く、カーソルが離れたら閉じる、開かない理由）と `tests/components/jev-suggest-link-command.test.ts` が固定する。実機は未実施（E19、LEV-173。手順は `artifacts/LEV-172-e2e/plan.md` と PR 本文）。
