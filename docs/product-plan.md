...
実測（2026-09-22、`artifacts/3d-3-measure/record.md`）: 75 ノートの fixture で 2D 68 ノード／546 要素／120 ms、3D 49 ノード／200 要素／149 ms、箱の重なり 0、console.error なし。zoomToFit は床を除いて小さな fixture で 2D と同程度（0.587 → 0.566）、大きな fixture は Up の 5 列が広く 0.24。残る子は LEV-123（埋め込みの中心）。 LEV-149 で帯の行間を画面基準に直した（`regridBand` に渡す `rowHeight` を `groundGapNorthSouth` で戻す。2D のままだと画面の行間が 23px で箱の高さ 66px より小さく、行が重なっていた）。実測（2026-09-22、`artifacts/3d-band-rows-e2e/record.md`、本人が親 27・子 37 まで増やした状態）: 段をまたぐものも含めて実際に交差する箱が 25 組 → 15 組。残る 15 組は Up／Down が床の縁を追い越す件（帯が伸びると床が画面で上下に広がるのに軸の高さが固定値）と、友・兄弟の帯の行間が未補正な件。
