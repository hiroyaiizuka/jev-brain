...
現在の実装（LEV-170）: コマンド `excalibrain-jev-type-link`（英語 UI で "Jev: type the link at the cursor (confirm the first candidate)"、日本語 UI で「Jev: カーソルのリンクに型を付ける（第一候補で確定）」）を `registerJev()` が登録し、`src/jev/typeLink.ts` の `typeLinkAtCursor` が中核を順に通す（カーソルの `[[X]]` を本文の書き方のまま取り → `collectUntypedLinks` で未型付けか確かめ → `buildState`／`buildQuestions` → `askJev` → `judge` → `confident` なら `appendRelation` と `appendLogEntry`（`source: "command"`）、自信なしは確率を伏せた候補を Notice に出して何も書かない）。書くのも聞くのも本文の `[[X]]` のとおりの名前で（解決したパスと別名は使わない）、埋め込み・型付き・カーソルがリンクの外は何もしない。UI は `src/Components/JevTypeLinkCommand.ts` の Notice 1 本だけで、文言は `en`／`ja` の `JEV_COMMAND_*`。JevBrain を一度も開いていない Vault では索引をここで作る。`tests/jev/typeLink.test.ts`（9 件）が client をスタブにして、追記・記録・送った state と 2 問・別名・未解決・自信なし・応答なし・重複・カーソル外・索引なしを固定する。実機（E18）は本人の API キー待ちで未実施。手順書は `artifacts/jev-1-e2e/plan.md`。
