> **jev-brain**: [ExcaliBrain](https://github.com/zsviczian/excalibrain)（MIT、Zsolt Viczián 作）のフォーク。Jev による支援と 3D 表示を加える構想で、現在は上流 0.2.18 に開発ハーネスを足した段階。plugin ID は `jevbrain`、名前は JevBrain なので上流版（`excalibrain`）と同時にインストールできる。command ID・CSS クラス・設定のキー・画面の表示名は上流のまま。開発の進め方は末尾の「開発」と `AGENTS.md`、`docs/` を参照。以下は上流の README。
